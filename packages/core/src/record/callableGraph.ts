...
export class CallableGraph {
  ...
  removeNode<
    ...
    CallOptions extends CallableConfig = CallableConfig,
  >(callableNode: CallableNode<CallInput, CallOutput, CallOptions>) {
    const nodeId: RecordId = callableNode.callable.getNodeId();
    if (this.getNode(nodeId)) {
      this._nodes.delete(callableNode);
...
function isCyclicUtil(
  ...
  if (!visited.has(node)) {
    ...
    for (const dependent of Array.from(node.dependents.keys()).map((k) =>
      node.dependents.get(k)
    )) {
      if (
        dependent &&
        ...
        return true;
      } else if (dependent && recStack.has(dependent)) {
        return true;
      }
    }
  }
