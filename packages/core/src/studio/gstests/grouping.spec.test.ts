...
test('test assign nodes to two workers', async () => {
  ...
  const connections = [connAB, connBC,connBD, connDE, connEF];
  ...
  // expect(criticalPathFunction).toMatchSnapshot();
  // expect(groupSet.length).toEqual(2);
  // expect(groupSet[0].length).toEqual(3);
  expect(criticalPathFunction.length).toEqual(5);
});
