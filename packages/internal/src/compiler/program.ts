...
export function createSourceFile(
  ...
  const sourceFile = api.createSourceFile(fakeFile, code, scriptTarget, false, scriptKind);
  ...
  function getBindingResult() {
    ...
    const compilerHost: ts.CompilerHost = {
      ...
      getDefaultLibFileName: 
        (defaultLibOptions: ts.CompilerOptions) => "/" + api.getDefaultLibFileName(defaultLibOptions),
      writeFile: () => {
        // do nothing
      },
      getCurrentDirectory: () => "/",
